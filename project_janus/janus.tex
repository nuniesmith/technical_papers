...
\begin{itemize}
    \item \textbf{Visual Pattern Recognition} using Gramian Angular Fields (GAF) \citep{wang2015imaging} and Video Vision Transformers (ViViT) \citep{arnab2021vivit}
    \item \textbf{Symbolic Reasoning} via Logic Tensor Networks (LTN) \citep{badreddine2022logic} for constraint satisfaction
    \item \textbf{Multimodal Fusion} integrating time series \citep{ansari2024chronos}, visual, and textual market data
    \item \textbf{Dual-Pathway Decision Making} inspired by basal ganglia architecture \citep{collins2014opponent}
\end{itemize}

The Forward service operates on a hot path with strict latency requirements, implementing end-to-end gradient flow through differentiable market simulation \citep{fu2024jaxlob} while maintaining regulatory compliance through symbolic constraints. FPGA acceleration \citep{marino2023mevit, vemeko2023fpga} enables nanosecond-level latency for high-frequency trading applications.
...
The GAF video is processed by a factorized spatiotemporal transformer \citep{arnab2021vivit}. Unlike standard Vision Transformers \citep{dosovitskiy2020image} which process static images, ViViT factorizes attention across both space (price/volume levels of the limit order book) and time (sequences of LOB snapshots), enabling the system to track dynamic microstructure events.
...
The Wash Sale Rule \citep{irs2024wash}—a critical regulatory constraint for active traders—prevents claiming tax losses on securities sold and repurchased within 30 days:
\begin{equation}
...
\begin{itemize}
    \item Visual: $\mathbf{v} \in \mathbb{R}^{d_v}$ (from ViViT \citep{arnab2021vivit})
    \item Temporal: $\mathbf{t} \in \mathbb{R}^{d_t}$ (from Chronos \citep{ansari2024chronos} or Transformer \citep{vaswani2017attention})
    \item Textual: $\mathbf{s} \in \mathbb{R}^{d_s}$ (from BERT embeddings for news/sentiment)
\end{itemize}
...
Following the attention mechanism of \citet{vaswani2017attention}:
\begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{Schema Formation} via UMAP-based clustering \citep{mcinnes2018umap, alignedumap2023}
...
Schemas are stored in Qdrant \citep{qdrant2024}, a high-performance vector similarity search engine, enabling fast retrieval of relevant historical patterns.
...
This mapping is grounded in empirical neuroscience and cognitive modeling \citep{frank2006working, collins2014opponent, foster2013hierarchical}.
...
FPGA acceleration using AMD Alveo U55C cards \citep{amd2023alveo, vemeko2023fpga, marino2023mevit} enables the following targets:
...
The Thalamus functions as the "gatekeeper" of perception in JANUS, regulating the flow of visual and numerical data into the decision engine. This is implemented using Wilson-Cowan mean-field models \citep{wilson1972excitatory, wilson2024bidirectional}, which simulate the oscillatory dynamics of neural populations.

These models generate "attention masks" that suppress noise (irrelevant price ticks) and amplify signal (structural breaks). The Thalamic Reticular Nucleus provides attentional gating \citep{thalamus2018attention, halassa2014thalamic}, enabling JANUS to focus computational resources on the most informative market data streams. The integration of gated cross-attention \citep{gatedattention2023} serves as the machine-learning equivalent of this biological process.
...
\begin{itemize}
    ...
    \item Integration of foundation models for time series \citep{ansari2024chronos, ansari2024chronos2}
