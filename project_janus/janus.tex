...
\usepackage{float}             % enables the [H] float specifier to lock tables in place
...
\hypersetup{
    ...
    urlcolor=janusblue,
    citecolor=janusblue,
    pdftitle={Project JANUS: A Neuromorphic Architecture for Autonomous Trading},
...
% --- TITLE PAGE ---
% Single-spaced and compact so the whole title page (through the date and
% epigraph) fits on ONE page — the document's \onehalfspacing previously
% pushed the Author/Date block onto a nearly-empty second page.
\begin{titlepage}
	\singlespacing
	\centering
	\vspace*{0.4cm}
	...
	\begin{tcolorbox}[colback=janusblue!5, colframe=janusblue, title=\textbf{Unified Documentation}, width=0.85\textwidth]
		...
		\begin{enumerate}[leftmargin=1.5cm, itemsep=1pt, parsep=1pt, topsep=2pt]
			\item \textbf{\textcolor{janusblue}{Introduction}} — The Quant 4.0 paradigm, crowding resistance, and contributions
	...
	{\large\bfseries Jordan Smith\par}
	\vspace{0.15cm}
...
\subsection{A Secondary Motivation: Strategy Crowding and Co-Impact}
\label{sec:crowding_motivation}

The Quant 4.0 transition carries a structural risk of its own: as firms converge on similar architectures and pre-trained foundation models, output homogeneity increases regardless of nominal strategy differences. This motivates JANUS's second design concern. Strategy crowding---the convergence of multiple algorithmic trading systems on similar positions---poses systemic risk through amplified co-impact costs and correlated drawdowns.
...
\begin{enumerate}
	...
	\item \textbf{Neuro-symbolic fusion:} The first application of Logic Tensor Networks \citep{badreddine2022logic} to financial decision-making, formulating regulatory and risk constraints as differentiable fuzzy-logic axioms.
	\item \textbf{Opponent Actor Learning for trading:} A design for financial action selection based on basal ganglia computational models \citep{collins2014opponent, jaskir2023opal}---to our knowledge the first such application to trading.
...
Action selection in JANUS is designed to be mediated by a model of the Basal Ganglia, comprising Direct (``Go'') and Indirect (``No-Go'') pathways \citep{collins2014opponent, foster2013hierarchical}. This architecture, known as Opponent Actor Learning (OpAL), creates dynamic risk tolerance that adapts to market conditions.
...
The system comprises eight services plus supporting infrastructure (see Figure in Section~\ref{sec:topology} for the component diagram). In the current production deployment these run as modules of a single unified binary: a supervisor process spawns the Data, Forward, Backward, CNS, and API modules in-process (with per-module health reporting and restart policies), which eliminates inter-service serialization on the hot path while preserving the logical boundaries below. The standalone-service topology remains supported for horizontally scaled deployments.
...
\begin{enumerate}
	...
	\item \textbf{Optimizer Service:}
	      Hyperparameter optimization with backtesting integration (metrics on the unified \texttt{:9090} endpoint in single-binary mode).
\end{enumerate}
...
\begin{itemize}
	...
	\item \textbf{Prometheus} (port 9090): Metrics collection with a curated alert-rule set, audited so that every rule references an exported metric (data-quality SLOs, host safety, per-bot performance)
	\item \textbf{Grafana} (port 3000): Visualization dashboards (signal pipeline, data-quality SLOs, operations, per-bot performance)
	...
	\item \textbf{Nginx}: Reverse proxy with TLS termination and internal-token injection (deployments sit behind a WireGuard-based private network)
\end{itemize}
...
\subsection{Research-to-Production Maturity Gap}
\label{sec:maturity_gap}

The subsystems described in this paper differ in deployment maturity, and we state the gap plainly. The live trading path is a deterministic regime/rule engine---technical indicators $\rightarrow$ regime detection $\rightarrow$ homeostatic position scaling $\rightarrow$ threat filtering $\rightarrow$ execution gating---and the brain wiring pipeline (Section~\ref{sec:basal_ganglia}) that orchestrates it is rule-based rather than neural. Its safety, compliance, and signal-processing infrastructure (kill switch, wash-sale/prop-firm compliance, VPIN flow-toxicity, the fractal DSP pipeline, ensemble regime detection, and Wilson-Cowan thalamic dynamics) is implemented and live, as is its observability layer: generated signals are persisted from the in-process signal bus to Redis for the read APIs and operator UI, and a data-completeness SLI computed in the live ingestion path backs a data-loss SLO alert (which, on its first armed day, surfaced a silently dead exchange stream caused by an upstream ticker migration). By contrast, the neuromorphic and deep-learning subsystems that constitute the cognitive core---OpAL action selection (Section~\ref{sec:basal_ganglia}), the LTN constraint engine (Section~\ref{sec:ltn}), the DiffGAF/ViViT vision stack (Section~\ref{sec:perception}), and the three-timescale memory hierarchy (Section~\ref{sec:memory})---are implemented as standalone modules and validated in isolation, but are not yet integrated into the live inference loop, and no trained model weights currently drive live decisions. Wiring these modules into the hot path, exporting trained weights, and validating end-to-end behavior is the primary thrust of ongoing work; Section~\ref{sec:implementation_status} gives a per-component breakdown of maturity.
...
\textbf{Mitigations:} JANUS adopts a dual-mode LTN design: strict mode (full axiom evaluation) for the cold path and approximate mode (reduced axiom set with cached evaluations) for the hot path.
...
The primary validation of JANUS's crowding-resistance claim requires multi-agent simulation using the Rust-native limit order book simulator \citep{fu2024jaxlob} (CPU matching engine; GPU acceleration is planned).
...
\begin{enumerate}
	...
	\item \textbf{Neuro-Symbolic Fusion:} LTN constraint engine with domain-specific axioms and linguistic hedges bridging neural networks and logical constraints \citep{badreddine2022logic}
	...
	\item \textbf{OpAL Decision Engine (design):} An Opponent Actor Learning module for financial action selection \citep{collins2014opponent, jaskir2023opal}---to our knowledge the first application of a basal ganglia computational model to this problem
	...
	\item \textbf{Production-Ready Safety:} Dual-layer kill switch (in-process + Redis) with four breaker actions (freeze, safe-mode, cancel-all), multi-method anomaly detection, FNI-RL fear network, and CNS health monitoring with five-phase preflight
	...
	\item \textbf{GPU Compute Infrastructure:} WGSL compute kernels (MatMul, Softmax, LayerNorm, Attention, GELU, Reduce) with a Candle tensor bridge---authored with CPU-reference execution; GPU dispatch pending
\end{enumerate}
...
Implementation maturity varies by subsystem, and we summarize it here rather than presenting every component as live. The production trading path is a deterministic regime/rule engine (technical indicators $\rightarrow$ regime detection $\rightarrow$ homeostatic position scaling $\rightarrow$ threat filtering $\rightarrow$ execution gating); its safety, compliance, and signal-processing infrastructure is implemented and live. The neuromorphic and deep-learning subsystems that constitute the cognitive core (Sections~\ref{sec:perception}--\ref{sec:memory})---notably OpAL action selection, the LTN constraint engine, and the DiffGAF/ViViT vision stack---are implemented as standalone modules and validated in isolation, but are not yet wired into the live inference loop; integrating them is ongoing work (Section~\ref{sec:maturity_gap}). The table below describes each component at its current maturity, annotated as live, standalone, or scaffolded/planned where the distinction matters.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|l|L|}
		...
		\hline
		Vision             & DiffGAF $\rightarrow$ ViViT pipeline (dual-GAF)---standalone, not yet in live loop                                                           \\
		\hline
		...
		\hline
		Forecasting        & Chronos tokenizer \citep{ansari2024chronos, ansari2024chronos2} with statistical-baseline forecasting (ONNX serving scaffolded, not wired)                                                \\
		\hline
		...
		\hline
		Synthetic Data     & Regime-conditional DDPM diffusion models \citep{coletta2024conditional} (Min-SNR/EMA scaffolded)                                                 \\
		\hline
		LOB Simulator      & CPU matching engine \citep{fu2024jaxlob} with Almgren-Chriss impact (GPU acceleration planned)                                         \\
		\hline
...
\begin{tcolorbox}[colback=janusblue!5, colframe=janusblue, title=\textbf{Repository \& Contact}]
	\centering
	\textbf{GitHub:} \url{https://github.com/nuniesmith/janus}
