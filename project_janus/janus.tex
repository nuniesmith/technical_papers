...
\textbf{Note:} The detailed mathematical specifications for each component are presented in Parts 2-5 below.
...
\begin{itemize}
    \item \textbf{Visual Pattern Recognition} using Gramian Angular Fields (GAF) \citep{wang2015imaging} with LSTM temporal modeling, and a planned upgrade path to Video Vision Transformers (ViViT) \citep{arnab2021vivit}
...
For the training pipeline (Python/PyTorch), we employ learnable affine transformations with domain constraints:
\begin{equation}
...
\subsubsection{Current: DiffGAF-LSTM}

The production vision model pairs the DiffGAF encoding with an LSTM temporal model. GAF frames are encoded and fed sequentially into an LSTM network that captures inter-frame temporal dependencies. This architecture provides robust performance while maintaining the differentiability required for end-to-end training.

\subsubsection{Planned: Video Vision Transformer (ViViT)}

The planned upgrade path uses a factorized spatiotemporal transformer \citep{arnab2021vivit}. Unlike standard Vision Transformers \citep{dosovitskiy2020image} which process static images, ViViT factorizes attention across both space (price/volume levels of the limit order book) and time (sequences of LOB snapshots), enabling the system to track dynamic microstructure events.
...
The ViViT architecture is designed to consume ONNX-exported models from the Python training pipeline, with graceful degradation to the DiffGAF-LSTM model when the ViViT model is unavailable.
...
The gating function suppresses noise and amplifies signal, similar to thalamic regulation \citep{halassa2014thalamic}:
\begin{equation}
...
\begin{equation}
\mathcal{D}_{\text{hippo}} = \{(s_t, a_t, r_t, s_{t+1}, \mathbf{c}_t, \mathbf{e}_t)\}_{t=1}^{T}
\end{equation}
...
\hline
Thalamus & Attentional gating \citep{halassa2014thalamic} & Modality fusion and signal routing \\
\hline
...
The visual cortex processes market data as images through the DiffGAF pipeline (Section~\ref{sec:visual_forward}), with submodules for GAF encoding (GASF/GADF), vision model inference (DiffGAF-LSTM, planned ViViT), data ingestion and buffering, and UMAP-based visualization of learned representations.
...
The Thalamus functions as the ``gatekeeper'' of perception in JANUS, regulating the flow of visual and numerical data into the decision engine \citep{thalamus2018attention, halassa2014thalamic}.
...
The basal ganglia implements Opponent Actor Learning (OpAL) \citep{collins2014opponent}, balancing Go (Direct) and No-Go (Indirect) pathways modulated by dopamine \citep{dopamine2020reward}. See Section~\ref{part:forward} for the complete mathematical formulation.
...
\begin{itemize}
    \item \textbf{ML Framework Strategy} leveraging tch-rs \citep{mazare2024tch}, Candle \citep{candle2024}, and ONNX Runtime
...
The move to Rust is justified by requirements in high-frequency trading systems \citep{tradfi2023hft}:
...
\begin{enumerate}
    ...
    \item \textbf{Ecosystem:} Production-ready ML inference via tch-rs \citep{mazare2024tch} and Candle \citep{candle2024}
\end{enumerate}
...
The following frameworks are evaluated for JANUS deployment \citep{mazare2024tch, candle2024}:
...
\hline
tch-rs \citep{mazare2024tch} & Native PyTorch, GPU support & C++ deps, larger binary & Training \& inference \\
\hline
ONNX Runtime & Universal, production-ready & No training & Inference only \\
\hline
Candle \citep{candle2024} & Pure Rust, HF integration & Young ecosystem & Future migration \\
\hline
...
\subsection{Current State and Migration Path}

The system is currently in \textbf{Phase 1 (Hybrid)}, with Candle already integrated for select components:

\subsubsection{Phase 1: Hybrid (Current)}
\begin{itemize}
    \item Train models in PyTorch (Python)
    \item Export to ONNX format
    \item Rust inference via \texttt{ort} and \texttt{tract} crates
    \item Candle integrated for select neural network operations
\end{itemize}

\subsubsection{Phase 2: Rust-Native Inference}
\begin{itemize}
...
\subsubsection{Phase 3: Full Rust ML}
\begin{itemize}
...
\begin{itemize}
    \item Database connectivity and schema integrity
    \item Exchange API authentication and rate limit status
    \item Model file availability and version consistency
    \item Memory and CPU resource adequacy
    \item Kill switch state verification
    \item Market data feed health
\end{itemize}
...
\begin{itemize}
    \item \textbf{Watchdog Monitoring:} Heartbeat-based service liveness detection
    \item \textbf{Boot Reports:} Comprehensive system state summary on startup
    \item \textbf{Prometheus Metrics:} 15+ custom metrics exposed for Grafana dashboards
    \item \textbf{Alert Integration:} Discord and Slack notifications for critical events
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Dependencies: ONNX model files, Redis, PostgreSQL
...
\begin{itemize}
    ...
    \item \textbf{Prometheus} (port 9090): Metrics collection
    \item \textbf{Grafana} (port 3000): Visualization dashboards (5+ dashboards)
    \item \textbf{Alertmanager} (port 9093): Alert routing
...
% ============================================
\newpage
\section{Advanced Neuroscience Integration}
\label{sec:advanced_neuro}

\subsection{Thalamic Oscillations and Attentional Gating}
...
Currently, the Thalamus module implements attentional gating through multi-head cross-attention with saliency computation and gating mechanisms (Section~\ref{sec:hypothalamus}). The Thalamic Reticular Nucleus provides attentional gating \citep{thalamus2018attention, halassa2014thalamic}, enabling JANUS to focus computational resources on the most informative market data streams.

\subsection{Dopaminergic Modulation and Market Regimes}
...
\subsection{Fear Extinction and Adaptive Risk Management}
...
\begin{enumerate}
    \item \textbf{Neuromorphic Architecture:} Ten biologically plausible brain regions with modular design
    \item \textbf{Neuro-Symbolic Fusion:} LTNs with linguistic hedges bridge neural networks and logical constraints
    \item \textbf{Multi-Timescale Memory:} Three-tier hierarchy with sleep-phase consolidation mirrors hippocampal-neocortical transfer
    \item \textbf{Ensemble Regime Detection:} HMM, statistical, and technical methods fused for robust market state identification
    \item \textbf{Production-Ready Safety:} Dual-layer kill switch, multi-method anomaly detection, and CNS health monitoring
    \item \textbf{Production-Ready Rust:} High-performance, safe, and maintainable five-service architecture
\end{enumerate}

\subsection*{Future Work}

\begin{itemize}
    \item ViViT spatiotemporal transformer upgrade for the vision pipeline
    \item Wilson-Cowan thalamic oscillation models for attention dynamics
    \item Chronos \citep{ansari2024chronos, ansari2024chronos2} integration for foundation model time series forecasting
    \item BERT text embeddings for structured sentiment analysis
    \item Parametric UMAP for real-time schema monitoring
    \item Quantum computing integration for portfolio optimization \citep{quantum2024portfolio}
    \item Continual learning without catastrophic forgetting using CLS principles \citep{mcclelland1995complementary}
    \item Multi-agent systems for distributed trading using feudal hierarchies \citep{vezhnevets2017feudal, feudal2019multi}
    \item Regulatory AI for automated compliance leveraging LTN constraint satisfaction \citep{badreddine2022logic}
    \item Hardware acceleration using FPGAs \citep{marino2023mevit, vemeko2023fpga, amd2023alveo} and neuromorphic chips
    \item Enhanced market microstructure analysis using VPIN and flow toxicity metrics \citep{easley2011vpin, easley2012flow}
    \item Generative diffusion models for synthetic market data generation \citep{diffusion2024lob}
    \item Scalable training with GPU-accelerated simulators \citep{fu2024jaxlob}
    \item Full Qdrant integration as primary vector database for schema storage
\end{itemize}
