...
\begin{itemize}
    \item \textbf{ML Framework Strategy} leveraging Candle \citep{candle2024} and tch-rs \citep{mazare2024tch} for end-to-end Rust-native ML
...
\begin{enumerate}
    ...
    \item \textbf{Ecosystem:} End-to-end ML training and inference via Candle \citep{candle2024} and tch-rs \citep{mazare2024tch}
...
The following Rust-native frameworks comprise the JANUS ML stack \citep{mazare2024tch, candle2024}:
...
\hline
Candle \citep{candle2024} & Pure Rust, HuggingFace integration, minimal deps & Younger ecosystem & Primary training \& inference \\
\hline
tch-rs \citep{mazare2024tch} & Mature PyTorch backend, full GPU support & C++ libtorch dependency & GPU-intensive training \& inference \\
\hline
...
\begin{itemize}
    \item End-to-end training and inference in Rust via Candle and tch-rs
    ...
    \item GPU acceleration via CUDA (tch-rs) and wgpu (Candle)
...
Currently, the Thalamus module implements attentional gating through multi-head cross-attention with saliency computation and gating mechanisms (Section~\ref{sec:thalamus}). The Thalamic Reticular Nucleus provides attentional gating \citep{thalamus2018attention, halassa2014thalamic}, enabling JANUS to focus computational resources on the most informative market data streams.
...
\begin{enumerate}
    ...
    \item \textbf{Pure Rust Stack:} End-to-end training and inference in Rust with high-performance, safe, and maintainable five-service architecture
\end{enumerate}
