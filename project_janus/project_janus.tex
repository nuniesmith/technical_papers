...
\begin{itemize}
    \item \textbf{The Cortex (HRM):} Hierarchical Reasoning Module with CEO/Worker architecture
    \item \textbf{The Oracle (Chronos-2):} Zero-shot probabilistic forecasting with Chronos-2 foundation model
    \item \textbf{The Eyes (CNN):} Visual pattern recognition via feature tensor encoding and Gramian Angular Fields (GAF)
    \item \textbf{The Conscience (HMM):} Regime detection via Hidden Markov Model
    \item \textbf{Product-not-Sum Consensus:} Multiplicative consensus mechanism with veto power
    \item \textbf{Multimodal Fusion (MSGCA):} Gated cross-attention for text/time series/vision fusion with FinBERT
    \item \textbf{Logic Tensor Networks (LTN):} Neuro-symbolic constraint enforcement with formal logical traces
    \item \textbf{Almgren-Chriss Execution:} Optimal execution framework with goal vector representation
    \item \textbf{Grad-CAM Visualization:} Visual evidence generation for CNN pattern recognition
    \item \textbf{MARL Framework:} Multi-Agent Reinforcement Learning for collective behavior modeling
    \item \textbf{Cooperative Logic:} LTN constraints for market stability
    \item \textbf{Validation Framework:} Constraint violation tracking, zero-shot generalization, market impact simulation
    \item \textbf{Glass Box Transparency:} Reasoning traces, motor scores, LTN traces, and visual evidence
\end{itemize}
...
JANUS supports multiple visual encoding approaches for time series data, enabling flexible pattern recognition through CNNs.

\textbf{Feature Tensor Encoding:} Market data is encoded as multi-dimensional feature tensors, transforming raw OHLCV data into a normalized matrix of shape $(sequence\_length, num\_features)$ where features include:
\begin{itemize}
...
\textbf{Gramian Angular Fields (GAF):} JANUS also implements GAF encoding as an advanced visual encoding technique. GAF transforms time series into polar coordinate images where temporal correlations are explicitly mapped to spatial dimensions. A market in a strong, low-volatility trend appears in a GASF image as a coherent, smooth gradient. Volatile markets manifest as chaotic ``checkerboard'' textures, providing rich visual texture analysis.

The CNN architecture processes both encoding formats, recognizing visual patterns in the feature space and distinguishing between different market regimes and volatility structures. Unlike traditional technical indicators that simply measure the magnitude of volatility, the CNN can classify the quality and structure of volatility patterns, identifying consolidation phases versus liquidity crashes.

\textbf{Visual Evidence:} Grad-CAM (Gradient-weighted Class Activation Mapping) generates heatmaps showing which input regions triggered CNN predictions, providing visual evidence for explainability queries.
...
\begin{itemize}[itemsep=0.1em, topsep=0.2em]
    ...
    \item \textbf{Visual Encoders:} CNN pre-trained on feature tensors and GAF images for pattern recognition.
\end{itemize}
\vspace{-0.3em}
...
\vspace{-0.2em}
\begin{itemize}[itemsep=0.1em, topsep=0.2em]
    \item \textbf{Phase 1 (Worker):} Worker trained in isolation using Almgren-Chriss objective with randomized goals and risk parameters $\lambda$.
    \item \textbf{Phase 2 (Manager):} Manager trained to optimize high-level PnL with frozen Worker, learning to generate optimal Goal Vectors $g_t$ and risk parameters.
    \item \textbf{Phase 3 (Joint Fine-Tuning):} Both agents trained end-to-end with logical loss $\mathcal{L}_{logic}$ added to reward signal, ensuring constraint satisfaction.
\end{itemize}
\vspace{-0.3em}
